...
static void display_flush(lv_display_t *display, const lv_area_t *area, void *color_p)
{
    (void) area;
    (void) color_p;

    // RTK_LOGI("#", "x1=%d y1=%d x2=%d y2=%d\n", area->x1, area->y1, area->x2, area->y2);
    ...
    //     }
    // }
    for (size_t j = 0; j < sizeof g_buffer_0; j++) {
        uint16_t data = g_buffer_0[j];
        data |= 0x100;
        g_line_buf[i++] = data;
        if (i == g_line_buf_count) {
            spi_master_write_stream_dma(&spi_lcd, g_line_buf, 2 * g_line_buf_count);
...
void lv_ameba_hal_init(void)
{
    ...
    lv_display_set_buffers(display, g_buffer_0, NULL, WIDTH * HEIGHT * 2, LV_DISPLAY_RENDER_MODE_FULL);
